...
public static function parseBbcode($data, $down_perm, $user_name, $msg_id = NULL) {
	...
	foreach ($smilies as $key => $value) {
		$pattern = '/(?<=[^\p{L}\p{N}]|^)(' . preg_quote($value, '/') . ')(?=[^\p{L}\p{N}]|$)/ui';
		$output = preg_replace(
			$pattern,
			self::popTemplate(
	...
	*/
    // Highlight current user references within the text
    $output = str_replace(
        '@'.$user_name, 
        self::popTemplate(
	...
	*/
	public static function parseBBcodeThumb($text) {
		...
		$imgTagRegex = '/\[IMG\](.*?)\[\/IMG\]/i';
		$youtubeRegex = '/https:\/\/www\.youtube\.com\/watch\?v=([0-9a-zA-Z-+_=]*)/';

		if (!preg_match($imageRegex, $text) && !preg_match($imgTagRegex, $text) && !preg_match($youtubeRegex, $text)) {
			// No images or videos detected, return trimmed text
			return trim($text);
		}

		// Image(s) or video detected, parse and replace
		$text = preg_replace_callback(
			$imageRegex,
			'WcImg::parseImg',
			trim($text)
